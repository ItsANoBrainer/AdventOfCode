...
const inputGroupArray = input.split(`\n\n`)

var totalChars = 0

inputGroupArray.forEach(group => {
    const totalUsers = group.split("\n").length
    const letterString = group.replace(/\n/g,'')
    const letterArray = []

    for(i=0;i<letterString.length;i++) {
        const currentChar = letterString.charAt(i)
        if(!letterArray.includes(currentChar)) { letterArray.push(currentChar) }
    }

    letterArray.forEach(char => {
        if(letterString.split(char).length -1 == totalUsers) { totalChars++ }
    })
})
