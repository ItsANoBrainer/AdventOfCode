...
//console.log(`Number Surrounding: ${getNumSurrounding(testInputArray, 2, 3)}`)
            ...
            for (j = 0; j < arrayCurrent[i].length; j++) {
                switch (getArrayCharAt(arrayCurrent, i, j)) {
                    case 'L':
                        ...
                        break;
                    case '#':
                        if (getNumSurrounding(arrayCurrent, i, j) >= 5) {
                            let arrayString = arrayNew[i].split('')
                            arrayString[j] = 'L'
                            arrayNew[i] = arrayString.join('')
                        }
...
function getNumSurrounding(array, y, x) {
    const directions=['N','S','E','W','NE','NW','SE','SW']
    let value = 0
 
    directions.forEach(direction => {
        if(getCharInDirection(array, y, x, direction) == '#') value++
    })

    return value
}

function getCharInDirection(array, y, x, direction) {
    const maxYIndex = array.length -1
    const maxXIndex = array[0].length -1

    let xOffset = 0
    let yOffset = 0
    
    if(direction.includes('N')) yOffset -= 1
    if(direction.includes('S')) yOffset += 1
    if(direction.includes('E')) xOffset += 1
    if(direction.includes('W')) xOffset -= 1

    //console.log(`Direction ${direction} | Starting at y:${y} x:${x} | Offset y:${yOffset} x:${xOffset}`)
    ...
    do {
        timesRan++
        const xPos = x+(xOffset*timesRan)
        const yPos = y+(yOffset*timesRan)
        isGoodX = (xPos >= 0 && xPos <= maxXIndex)
        isGoodY = (yPos >= 0 && yPos <= maxYIndex)
        if(isGoodX && isGoodY) {
            foundChar = getArrayCharAt(array, yPos, xPos)
        }
        //console.log(foundChar)
    } while(isGoodX && isGoodY && foundChar == '.')

    return foundChar
}

function getArrayCharAt(array, y, x) {
