...
const input = `LLLLL.LLLLLLLL..LLLLLLLLLLLLLL.LLLL..LL..LLLLLLLL.LLLL.LLLLLLLLLLLL.LLLLLL.LLLLLL.LLLLLLLL
...
console.log(evaluate(inputArray))

function evaluate(startingArray) {
    let arrayCurrent = startingArray.slice()
    let arrayNew = startingArray.slice()

    do {
        arrayCurrent = arrayNew.slice()
        for (i = 0; i < arrayCurrent.length; i++) {
            for (j = 0; j < arrayCurrent[i].length; j++) {
                switch (getArrayCharAt(arrayCurrent, i, j)) {
                    case 'L':
                        if (getNumSurrounding(arrayCurrent, i, j) == 0) {
                            let arrayString = arrayNew[i].split('')
                            arrayString[j] = '#'
                            arrayNew[i] = arrayString.join('')
                        }
                        break;
                    case '#':
                        if (getNumSurrounding(arrayCurrent, i, j) >= 4) {
                            let arrayString = arrayNew[i].split('')
                            arrayString[j] = 'L'
                            arrayNew[i] = arrayString.join('')
                        }
    ...
    } while (!compareArray(arrayCurrent, arrayNew))

    return countSeats(arrayCurrent)
}

function countSeats(array) {
    let value = 0
    for (i = 0; i < array.length; i++) {
        for (j = 0; j < array[i].length; j++) {
            if (array[i].charAt(j) == '#') value++
        }
    }
...
function getNumSurrounding(array, index, subIndex) {
    const maxHeightIndex = array.length -1
    const maxWidthIndex = array[0].length -1
    let value = 0
    //Get N
    if (index != 0 && getArrayCharAt(array, index - 1, subIndex) == '#') value++
    //Get S
    if (index != maxHeightIndex && getArrayCharAt(array, index + 1, subIndex) == '#') value++
    //Get W
    if (subIndex != 0 && getArrayCharAt(array, index, subIndex - 1) == '#') value++
    //Get E
    if (subIndex != maxWidthIndex && getArrayCharAt(array, index, subIndex + 1) == '#') value++
    //Get NE
    if (index != 0 && subIndex != maxWidthIndex && getArrayCharAt(array, index - 1, subIndex + 1) == '#') value++
    //Get NW
    if (index != 0 && subIndex != 0 && getArrayCharAt(array, index - 1, subIndex - 1) == '#') value++
    //Get SE
    if (index != maxHeightIndex && subIndex != maxWidthIndex && getArrayCharAt(array, index + 1, subIndex + 1) == '#') value++
    //Get SW
    if (index != maxHeightIndex && subIndex != 0 && getArrayCharAt(array, index + 1, subIndex - 1) == '#') value++

    return value
}

function getArrayCharAt(array, index, subIndex) {
    //console.log(`Getting char at ${index} ${subIndex}`)
    return array[index].charAt(subIndex)
}
