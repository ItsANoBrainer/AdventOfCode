...
arrayInput.forEach(passport => {
    const passportFields = passport.replace(/\n/g,' ').split(' ')
    let fieldBooleans = [false, false, false, false, false, false, false]

    passportFields.forEach(field => {
        const fieldName=field.split(':')[0]
        const fieldData=field.split(':')[1]

        switch(fieldName) {
            case 'byr':
                if(fieldData.length == 4 && fieldData >= 1920 && fieldData <= 2002) { fieldBooleans[0]=true }
                break;
            case 'iyr':
                if(fieldData.length == 4 && fieldData >= 2010 && fieldData <= 2020) { fieldBooleans[1]=true }
                break;
            case 'eyr':
                if(fieldData.length == 4 && fieldData >= 2020 && fieldData <= 2030) { fieldBooleans[2]=true } 
                break;
            case 'hgt':
                const unit = fieldData.slice(fieldData.length-2, fieldData.length)
                const num = fieldData.slice(0, fieldData.length-2)
                if(unit == "cm" && num >= 150 && num <= 193) { fieldBooleans[3]=true } 
                else if (unit == "in" && num >=59 && num <=76) { fieldBooleans[3]=true }
                break;     
            case 'hcl':
                const hcl = fieldData.slice(1,7)
                if(fieldData.charAt(0)=='#' && hcl.length==6 && hcl.match(/^[0-9a-f]+$/g) != null) { fieldBooleans[4]=true}
                break;   
            case 'ecl':
                const validColors = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']
                if(validColors.includes(fieldData)) { fieldBooleans[5]=true }
                break;       
            case 'pid':
                if(fieldData.match(/^[0-9]{9}$/g) != null) {fieldBooleans[6]=true}
                break; 
        }
    })

    if(!fieldBooleans.includes(false)) {validPassports++ }
})
