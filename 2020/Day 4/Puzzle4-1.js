...
arrayInput.forEach(passport => {
    const hasbyr  = passport.includes(`byr`)
    const hasiyr = passport.includes(`iyr`)
    const haseyr = passport.includes(`eyr`)
    const hashgt  = passport.includes(`hgt`)
    const hashcl  = passport.includes(`hcl`)
    const hasecl  = passport.includes(`ecl`)
    const haspid = passport.includes(`pid`)

    if(hasbyr + hasiyr + haseyr + hashgt + hashcl + hasecl + haspid == 7) { validPassports++ }
})
